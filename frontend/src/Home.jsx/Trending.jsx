...
const Trending = () => {
  ...
  return (
    <div className='mx-[10px] md:mx-[80px]'>
      <h1 className='text-3xl font-bold m-[20px]'>Trending</h1>
      
      {loading ? (
        <p>Loading blogs...</p> // Show a loading message or spinner
      ) : (
              ...
              <Link to={`/blogs/${item._id}`} key={item._id}>
                <div className='my-2 wfull mx-2 rounded-lg relative hover:scale-95 shadow-xl duration-150'>
                  ...
                  <div className='h-20 flex flex-row items-center'>
                    <img className='w-[50px] h-[50px] ml-[20px] rounded-full mr-[5px]' src={item.UserPhoto.url} alt="" />
